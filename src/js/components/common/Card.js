...
const Card = ({ data, sibling, size, setModalOn, setModalData }) => {
  ...
  const clickImage = (e) => {
    fetch(
      `https://h3rb9c0ugl.execute-api.ap-northeast-2.amazonaws.com/develop/baminchan/detail/${detail_hash}`
    )
      .then((response) => response.json())
      .then((json) => {
        if (!json.hash) return console.log("error!! : card click - ", json);
        json.data.name = title;
        json.data.hash = detail_hash;
        json.data.badge = badge;
        json.data.sibling = sibling;
        const modalHeight = 1050;
        json.data.y = Math.round(
          e.pageY - e.clientY + (window.innerHeight - modalHeight) / 2
        );
      ...
      <CardWrapper>
        ...
        >
          <CardImage
            ...
            isHover={isHover}
            onError={() =>
              setSrc("https://codesquad.kr/img/company/codesquad2.png")
            }
          />
          <CardHover size={size}>
            {isHover && delivery_type.map((e) => <li key={e}>{e}</li>)}
          </CardHover>
        </div>
        <CardInfo size={size}>
...
const CardImage = styled.img`
  ...
  border-radius: 5px;
  filter: ${(props) =>
    props.isHover ? "brightness(40%)" : "brightness(100%)"};
  margin-bottom: 16px;
`;
